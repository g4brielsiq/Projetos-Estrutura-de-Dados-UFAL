...
void inicializar_cozinha(Cozinha *cozinha)
{
    ...
    // Mapeamento dos 13 funcionários (versão do usuário)
...
void coletar_pedidos_do_usuario(Cozinha *c)
{
    ...
    {
        // --- CORREÇÃO AQUI ---
        // Usamos (i + 1) para o ID durante a criação, garantindo que seja único nesta fase.
        ...
        if (qtd_itens > 0)
        {
            ...
            novo_pedido->id = id_pedido_atual; // O ID correto é atribuído aqui
            ...
            c->total_pedidos_criados++; // O contador global só aumenta para pedidos válidos
...
void despachar_tarefas(Cozinha *c)
{
    ...
    for (int h = 0; h < 4; h++)
    {
        ...
        for (int i = c->num_tarefas_na_fila_preparo - 1; i >= 0; i--)
        {
            ...
            // Lógica de despacho refatorada para switch para maior clareza
                ...
                if (tarefa_concluida->tempo_conclusao <= cozinha->tempo_atual)
                {
                    ...
                    if (tarefa_concluida->tipo_item == ITEM_MONTAGEM)
                    {
                        ...
                        if (tempo_de_producao <= TEMPO_MAX_ATENDIMENTO)
                        {
                            ...
                            // --- MENSAGEM DE FINALIZAÇÃO CORRIGIDA ---
                            printf("\n>> Pedido #%d FINALIZADO com SUCESSO (%d seg)\n", pedido_pai->id, tempo_de_producao + TEMPO_ATENDIMENTO_CAIXA);
                        }
                        else
                        {
