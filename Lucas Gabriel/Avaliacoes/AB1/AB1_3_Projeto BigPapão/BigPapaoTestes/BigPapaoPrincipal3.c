...
// =============================================================================
// NOVA FUNÇÃO PARA IMPRIMIR AS BANDEJAS
// =============================================================================
...
void imprimir_composicao_bandejas(Pedido *pedido)
{
    ...
    // Arrays temporários para guardar os itens separados por tipo
    TipoItem itens_comer[pedido->num_itens];
    TipoItem itens_beber[pedido->num_itens];
    int count_comer = 0;
    int count_beber = 0;

    // Passo 1: Separa todos os itens do pedido em dois grupos
    for (int i = 0; i < pedido->num_itens; i++)
    {
        // Sanduíches e Batata são "de comer"
        if (pedido->itens[i] <= ITEM_BATATA_FRITA)
        {
            itens_comer[count_comer++] = pedido->itens[i];
        }
        // Refrigerante, Suco e Milk-shake são "de beber"
        else
        {
    ...
    int bandeja_num = 1;
    int idx_comer_atual = 0; // Índice para percorrer a lista de comidas
    int idx_beber_atual = 0; // Índice para percorrer a lista de bebidas

    // Passo 2: Loop principal que continua enquanto houver itens a serem alocados
    while (idx_comer_atual < count_comer || idx_beber_atual < count_beber)
    {
        printf("Bandeja %d:\n", bandeja_num++);

        // Passo 3a: Pega até 2 itens de comer da lista e imprime
        for (int i = 0; i < 2 && idx_comer_atual < count_comer; i++)
        {
            printf("  - %s\n", NOMES_ITENS[itens_comer[idx_comer_atual]]);
            idx_comer_atual++; // Avança o índice para o próximo item
        }

        // Passo 3b: Pega até 2 itens de beber da lista e imprime
        for (int i = 0; i < 2 && idx_beber_atual < count_beber; i++)
        {
            printf("  - %s\n", NOMES_ITENS[itens_beber[idx_beber_atual]]);
            idx_beber_atual++; // Avança o índice para o próximo item
        }
    }
    printf("---------------------------------\n");
}
