...
typedef enum {
    ...
    // Habilidades de Caixa e Separação não geram tarefas de produção, então foram omitidas aqui.
} Habilidade;
...
typedef struct {
    ...
    Pedido** pedidos_em_andamento; // Vetor dinâmico de ponteiros para Pedidos
...
void inicializar_cozinha(Cozinha* cozinha) {
    ...
    // VALORES FIXOS PARA A EQUIPE (RESULTADO DA PERSONALIZAÇÃO = 2, 2, 2)
...
// Função auxiliar para encontrar um pedido em andamento pelo ID
...
// NOVA LÓGICA DE ALOCAÇÃO EM LOTE - O CORAÇÃO DA SIMULAÇÃO
void despachar_tarefas(Cozinha* c) {
    Habilidade habilidades[] = {HABILIDADE_SANDUICHE, HABILIDADE_BATATA, HABILIDADE_BEBIDAS, HABILIDADE_SUCO, HABILIDADE_MONTAGEM};
    Equipamento* equipamentos[] = {&c->chapa, &c->fritadeira, &c->liquidificador, NULL, NULL};
    TipoItem tipos_min[] = {ITEM_SANDUICHE_SIMPLES, ITEM_BATATA_FRITA, ITEM_REFRIGERANTE, ITEM_SUCO, ITEM_MONTAGEM};
    TipoItem tipos_max[] = {ITEM_SANDUICHE_ELABORADO, ITEM_BATATA_FRITA, ITEM_MILK_SHAKE, ITEM_SUCO, ITEM_MONTAGEM};

    for (int h = 0; h < 5; h++) { // Itera sobre cada tipo de habilidade/tarefa
        // 1. Contar funcionários livres para esta habilidade
        ...
        if (num_funcs_livres == 0) continue; // Nenhum funcionário para esta tarefa, vai para a próxima

        // 2. Calcular capacidade total de produção para esta habilidade
        int capacidade_total = num_funcs_livres * (equipamentos[h] ? equipamentos[h]->capacidade_por_funcionario : 1);

        // 3. Alocar tarefas da fila de preparo
        ...
        for (int i = c->num_tarefas_na_fila_preparo - 1; i >= 0; i--) {
            ...
            if (tarefa_atual->tipo_item >= tipos_min[h] && tarefa_atual->tipo_item <= tipos_max[h]) {
                // Aloca a tarefa!
                ...
                // Move tarefa para a lista de "em execução"
...
void executar_simulacao(Cozinha* cozinha) {
    ...
    while (cozinha->total_pedidos_criados > (cozinha->atendidos_no_prazo + cozinha->atendidos_com_atraso)) {
        // ETAPA 1: Mover pedidos da fila de espera para "em andamento"
        if (cozinha->pedidos_na_fila_espera != NULL && cozinha->pedidos_na_fila_espera->tempo_chegada <= cozinha->tempo_atual) {
            ...
            for (int i = 0; i < pedido_iniciado->num_itens; i++) {
                Tarefa t = {pedido_iniciado->id, pedido_iniciado->itens[i]};
        ...
        // ETAPA 2: Despachar tarefas pendentes usando a nova lógica de lote
        despachar_tarefas(cozinha);

        // ETAPA 3: Avançar o tempo para o próximo evento
        if (cozinha->num_tarefas_em_execucao == 0) {
            if(cozinha->pedidos_na_fila_espera != NULL || cozinha->num_tarefas_na_fila_preparo > 0) {
                 cozinha->tempo_atual++;
                 continue;
            }
            break; // Sem tarefas em execução e sem tarefas pendentes, termina
        }
        ...
        // ETAPA 4: Processar tarefas concluídas
        for (int i = cozinha->num_tarefas_em_execucao - 1; i >= 0; i--) {
            ...
            if (tarefa_concluida->tempo_conclusao <= cozinha->tempo_atual) {
                ...
                if (!pedido_pai) continue; // Pedido já pode ter sido finalizado
                ...
                if (tarefa_concluida->tipo_item == ITEM_MONTAGEM) {
                    pedido_pai->status = STATUS_CONCLUIDO_NO_PRAZO; // Temporário
                    ...
                    if (pedido_pai->tarefas_preparo_restantes == 0 && pedido_pai->status == STATUS_EM_PREPARO) {
                        ...
                        Tarefa t = {pedido_pai->id, ITEM_MONTAGEM};
                        cozinha->num_tarefas_na_fila_preparo++;
                        cozinha->tarefas_na_fila_preparo = (Tarefa*)realloc(cozinha->tarefas_na_fila_preparo, sizeof(Tarefa) * cozinha->num_tarefas_na_fila_preparo);
                        cozinha->tarefas_na_fila_preparo[cozinha->num_tarefas_na_fila_preparo-1] = t;
                ...
                // Remove tarefa da lista de execução
